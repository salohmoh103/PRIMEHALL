...
# 1. إعدادات البيئة لضمان عمل الروابط بشكل آمن
...
# 2. تصحيح مسار قاعدة البيانات للعمل على Render (استخدام المجلد المؤقت)
...
class User(db.Model):
    ...
    points = db.Column(db.Integer, default=50) # 50 نقطة عند التسجيل
...
def callback():
    ...
    # 3. تعديل طريقة طلب البيانات لمنع خطأ JSONDecodeError
    response = requests.post('https://discord.com/api/oauth2/token', data=data, headers=headers)
    
    if response.status_code != 200:
        return f"Discord Auth Failed: {response.text}", response.status_code
...
def exchange_role():
    ...
    return jsonify({'success': False, 'message': 'نقاطك غير كافية!'})
...
def claim_gift():
    ...
    send_webhook("🎁 كود هدية جديد", f"👤 من الأيدي: {session['user_id']}\n🎫 الكود: `{code}`", 0xff6b6b)
