...
# 1. إعدادات البيئة لضمان عمل الروابط بشكل آمن وتجاوز قيود OAuth المحلية
os.environ['OAUTHLIB_INSECURE_TRANSPORT'] = '1'

app = Flask(__name__)

# 2. تفعيل الـ ProxyFix لحل مشكلة الـ HTTPS والروابط الخارجية على منصة Render
app.wsgi_app = ProxyFix(app.wsgi_app, x_proto=1, x_host=1)
app.secret_key = 'primhall_ultra_key_2024'

# 3. إعداد قاعدة البيانات - تم تغيير المسار إلى /tmp/ لضمان صلاحية الكتابة على Render
...
class User(db.Model):
    ...
    points = db.Column(db.Integer, default=50) # الرصيد الافتراضي هو 50 نقطة

# إنشاء قاعدة البيانات تلقائياً عند بدء التشغيل
...
def send_webhook(title, message, color=0x7289da):
    """إرسال إشعارات إلى قناة الديسكورد عبر الويب هوك"""
    try:
...
@app.route('/callback')
def callback():
    """معالجة البيانات العائدة من ديسكورد بعد تسجيل الدخول"""
    ...
    # إضافة الرأس (Header) لضمان قبول الطلب من سيرفرات ديسكورد
    ...
    try:
        ...
        return f"حدث خطأ برمي أثناء معالجة البيانات: {str(e)}", 500
...
def exchange_role():
    """معالجة طلبات استبدال النقاط برتب"""
    if not session.get('logged_in'): 
